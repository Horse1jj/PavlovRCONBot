    ...
    client.RCONCommandHandler(soc, `MapList`, [], msg.author).then(async (res) => {
        ...
        if (r && r.MapList) {
            ...
            const embed = new EmbedBuilder()
                .setTitle("Map Change Command")
                .setDescription(`${MAPS.map(m => `(**${m.num}**) __**${m.map}**__ ${m.mode}`).join("\n")}`)
                .setFooter({ text: `Say "STOP" to stop selection!` })
                .setColor(0x0099ff);
            ...
            msg_collector.on('collect', (m) => {
                let map = MAPS.find(s => s.num == m.content);
                if (map) {
...
exports.help = {
    ...
    name: "Map",
    view: "Map",
    desc: "Gives a list of maps to choose to change from (used the server's map rotation)."
};
